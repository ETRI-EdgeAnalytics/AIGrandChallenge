...
class MicroBlock(nn.Module):
    ...
    '''
    def __init__(self, in_planes, out_planes, expansion, layer_conf, stride, device, add_se = False, act = 'ReLU', bn = nn.BatchNorm2d ):
        ...
        self.shortcut1_add_quant = nn.quantized.FloatFunctional()
        self.shortcut1_mul_quant = nn.quantized.FloatFunctional()
        self.shortcut2_add_quant = nn.quantized.FloatFunctional()
    ...
    def forward(self, x):
        ...
        if self.add_se:
            ...
            if self.stride == 1:
                ...
                return  self.shortcut1_add_quant.add(self.shortcut1_mul_quant.mul(out,w) , self.shortcut(x))
            else:
                #out = out
                return  out
        ...
        if self.stride == 1:
            ...
            return  self.shortcut2_add_quant.add(out, self.shortcut(x))
        else:
            return out
...
class MicroNet(nn.Module):
    ...
    def __init__(self, ver = 'ver1', ver_new_cfg = None, run_type='train', num_classes=100, wide_factor = 1, depth_factor =1, add_se = True, 
        ...
        self.layers = self._make_layers(in_planes=self.input_channel,run_type=self.run_type)
    ...
    def _make_layers(self, in_planes, run_type):
        layers = []
        for expansion, out_planes, num_blocks, layer_conf in self.cfg:
            strides = [int(layer_conf[3][1])] + [1]*(int(num_blocks)-1)
            for stride in strides:
                layers.append(MicroBlock(in_planes, out_planes, expansion, layer_conf, stride, self.device, self.add_se, self.act, self.bn))
...
#model = MicroNet(ver="ver4", num_classes=41, add_se = True)
#print(model)

...
from model.micronet import MaskedConv2d, MaskedLinear, QuantConv2d, QuantLinear
...
register_hooks = {
    ...
    MaskedConv2d: count_convNd,
    MaskedLinear: count_linear,

    QuantConv2d: count_convNd,
...
def count(model, inputs, custom_ops=None, verbose=True):
    ...
    def add_hooks(m):
        ...
        if fn is None:
            if verbose:
                print("Count has not implemented counting method for ", m, fn)
        else:
            if verbose:
